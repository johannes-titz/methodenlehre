...
#' @parameter model lm model object
...
prepare_tovee <- function() {
  ...
  story <- paste0("In einer bekannten Studie untersuchten Tovée et al. (1999) wie weibliche Studierende die Attraktivität von Männern mit unterschiedlichen Körpermaßen bewerten. In der Studie kam heraus, dass der Waist-to-Chest-Ratio <i>wcr</i>, der beste Prädiktor für die Attraktivitätsbewertung <i>attract</i> ist. Der Waist-to-Chest-Ratio ist das Verhältnis zwischen Hüfte und Schultern (hier ausgedrückt in %). Sie finden einen Teil des Datensatzes im Internet, bestehend aus ", nrow(data), " bewerteten Männern.")
...
prepare_prestige <- function() {
  ...
  story <- paste0("Der <i>Prestige</i> Datensatz besteht aus 102 Berufen/Berufsgruppen eines Zensus in Kanada von 1971. Die Variablen sind unter anderem der durchschnittliche Bildungsgrad in Jahren (<i>education</i>), das durchschnittliche Einkommen in der Einheit 1000 USD pro Jahr (<i>income</i>), der Anteil an Frauen in % (<i>women%</i>) sowie der Pineo-Porter Prestige Score (<i>prestige</i>), der das Ansehen eines Berufs erfasst. Die Variablen beziehen sich alle auf die Berufsgruppe. ",
...
prepare_teaching_ratings <- function() {
  TeachingsRatnges <- AER::TeachingRatings
...
regression_example <- function(prepare_function, seed) {
    ...
    variables_md <- paste0("<i>", names(data), "</i>")
    model <- lm(data)
...
#' @sem standard error of measurement, which is not available in model #' and not generated by tab_model
...
prepare_html <- function(obj) {
    ...
    print(tab, file = "table.html")

    story_part2 <- "Beantworten Sie folgende Fragen. Wenn nötig, runden Sie bei numerischer Eingabe auf <b>3 Dezimalstellen</b>." 
    tbl <- paste(readLines("table.html"), collapse = "")
    c(obj$story, html_plot, tbl, story_part2)
}
...
encodeGraphic <- function(g) {
  ...
  png(tf1, width = 600, height = 500, res = 100)  # Get an unused filename in the session's temporary directory, and open that file for .png structured output.
...
get_questions <- function(parameters, variables_md) {
  ...
  df$feedback <- answers
  names(df) <- c("question", "solution", "points", "tolerance", "feedback")
  df$tolerance_type <- "relative"
  df
}

regression <- function() {
  tovee1 <- regression_example(sample(prep_functions(), 1), seed = sample.int(1e6, 1))
  content <- prepare_html(tovee1)
  q <- get_questions(tovee1$parameters, tovee1$variables_md)
  questions <- df2gap(q)
  cont <- append(content[1], questions)
  render_qtijs(new("Entry", content = cont))
}
